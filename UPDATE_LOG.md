...
## 2026-02-28 - Android新能力 & External Secrets Management

### 🟢 New Content（新增内容）

#### 1. Android设备状态查询 & 通知列表（第3章）
**更新位置**：`part-1/ch03.md` - 3.6节"Mobile Nodes"

**新增内容**（对应 2/27 Release）：
- ✅ `device_status` 命令：查询电池、存储、网络、屏幕等设备状态
- ✅ `device_info` 命令：获取设备型号、Android版本、Node版本
- ✅ `notifications_list` 命令：列出Android当前活跃通知，支持智能任务调度
- 附完整交互示例（4个use case）

#### 2. External Secrets Management（第7章 7.5.7节）
**更新位置**：`part-2/ch07.md` - 新增7.5.7节

**新增内容**（对应 2/27 Release）：
- ✅ `openclaw secrets audit` - 扫描配置中的明文密钥风险
- ✅ `openclaw secrets configure` - 交互式配置密钥
- ✅ `openclaw secrets apply` - 应用加密配置（带路径安全验证）
- ✅ `openclaw secrets reload` - 热重载密钥（无需重启）
- ✅ 迁移指南：旧版明文配置一键迁移到加密引用

### 📚 发现的新趋势（未写入书籍，记录备用）

#### 3. ACP Thread-bound Agents（2/27 Release）
ACP Agent已成为一等公民的thread session运行时，支持acp spawn/send分发、生命周期控制。
候选更新位置：`part-2/ch05-subagent.md`（ACP多Agent协作章节）

#### 4. Codex WebSocket-first Transport（2/27 Release）
openai-codex现在默认使用WebSocket传输（transport: "auto"，SSE作fallback）。
候选更新位置：`part-4/ch13.md`（模型选择章节）
