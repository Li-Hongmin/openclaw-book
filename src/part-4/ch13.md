...
| **openai-codex（WebSocket模式）** | 低延迟、实时流式 | 需要稳定网络 | 交互式编码、实时代码补全 |
...
### OpenClaw内置Token Dashboard

> 📅 更新于 2/26：OpenClaw v2026.2.6 引入了原生 Web UI Token Dashboard

如果你使用 OpenClaw v2026.2.6 或更高版本，无需自己实现监控系统——OpenClaw 已经内置了一个强大的 Token 使用仪表盘。

**启动 Web UI**：

```bash
# 启动 OpenClaw Gateway（默认会启用 Web UI）
  ...
  "gateway": {
    "webUI": {
      ...
      "dashboard": {
        "tokens": {
          "enabled": true,
          "retention": 90,  // 保留90天的历史数据
          "costEstimates": true  // 显示成本估算
        }
      }
    }
  }
...
Web UI 提供 API 端点，可以与你自己的监控系统集成：
...
response = requests.get(
    "http://localhost:3000/api/dashboard/tokens",
    params={"days": 7}
)
...
# 按agent分析
for agent in usage_data["agents"]:
    print(f"{agent['name']}: {agent['total_tokens']:,} tokens (${agent['cost']:.2f})")
```

**优势**：

- 📊 **开箱即用**：无需额外配置，安装即可使用
- 🔄 **自动同步**：所有 API 调用自动记录，无需手动埋点
- 📈 **专业可视化**：比自己用 Plotly 画图更美观、更专业
- 🔒 **安全访问**：支持基于 token 的认证，可以安全暴露给团队

> 💡 **提示**：Web UI Dashboard 适合日常监控和快速查看。如果需要更复杂的分析（如与 Prometheus/Grafana 集成），仍然可以使用上面的自定义 TokenTracker 实现。
...
## 🆕 Codex WebSocket-first 传输（2/27 Release）

> 📅 更新于 2026-03-01

OpenClaw 2/27 版本将 **openai-codex 的默认传输层升级为 WebSocket-first**（原来是 SSE）。这对实战中使用 Codex 进行交互式编码有明显影响。

### 传输层变化

| 版本 | 默认传输 | 回退机制 |
|------|---------|---------|
| 2/27 之前 | SSE（Server-Sent Events） | 无 |
| 2/27 之后 | WebSocket（`transport: "auto"`） | 自动回退到 SSE |

### 配置方式

```json5
// openclaw.json - Codex 模型配置
{
  models: {
    "openai-codex": {
      transport: "auto",       // 默认：WebSocket优先，SSE回退
      // transport: "websocket",  // 强制使用 WebSocket
      // transport: "sse",        // 强制使用 SSE（适用于代理不支持 WebSocket 的场景）
    }
  }
}
```
...
**WebSocket 的优势（交互式编码场景）**：
- ✅ **更低延迟**：双向持久连接，省去每次请求的握手开销
- ✅ **实时流式**：代码逐字符输出更流畅，不卡顿
- ✅ **连接复用**：多轮对话无需重新建立连接

**什么时候需要手动切换回 SSE**：
- ❌ 使用了不支持 WebSocket 的网络代理
- ❌ 防火墙屏蔽了 WebSocket 升级请求
- ❌ 部署在某些限制 WebSocket 的云平台

```bash
# 如果 Codex 连接异常，先检查传输层
openclaw doctor
# 输出示例：
# ✓ openai-codex: WebSocket transport OK
# ✗ openai-codex: WebSocket failed, using SSE fallback
```

> 💡 **最佳实践**：大多数情况下保持 `transport: "auto"` 即可，OpenClaw 会自动选择最优传输方式。只有在网络受限环境下才需要手动指定 `transport: "sse"`。
