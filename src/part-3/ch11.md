...
## 🔧 11.2 Self-healing模式深度实践[^self-healing-ch11]

现在让我们构建一个完整的自愈式服务器系统。这个案例会展示从健康监控到自动修复的完整流程,涉及真实的DevOps工具栈。

[^self-healing-ch11]: 案例来源：[Self-Healing Home Server](https://github.com/hesamsheikh/awesome-openclaw-usecases/blob/main/usecases/self-healing-home-server.md)，awesome-openclaw-usecases 社区贡献
...
## 🌐 11.5 实战：远程HPC Gateway部署

到目前为止，所有Agent都运行在本地机器上。但如果你有一台HPC超算（或任何远程服务器），能不能让Agent直接在那里运行？

答案是：**在远程机器上部署独立的OpenClaw Gateway**，通过Telegram Bot控制，同时与本地Gateway建立Node连接。

### 场景：学术研究者的HPC困境

你是一个大学研究者，有一台HPC超算的账号。日常工作流是：
...
**痛点**：每次都要SSH、记住路径、手写SLURM脚本。

**理想状态**：给Telegram Bot发消息"帮我把fastUMAP在MNIST数据集上跑一下"，Agent自动搞定一切。

### 架构设计

```mermaid
graph LR
    User[👤 你的手机] --> TG[📱 Telegram]
    
    TG --> GW1[🏠 Mac Studio Gateway<br/>port 18789]
    TG --> GW2[🖥️ HPC Gateway<br/>port 18790]
    ...
    A3 --> SLURM[SLURM<br/>Job调度]
    A3 --> GPU[🎮 A100 GPU]
    A3 --> FS[📂 /work/project/]
    
    style GW1 fill:#bbdefb,stroke:#333
    style GW2 fill:#c8e6c9,stroke:#333
```

**关键设计**：
- 两个**完全独立**的Gateway，各有自己的Telegram Bot
- HPC Gateway直接在超算上运行，可以执行SLURM命令
- 通过Node连接，本地Agent可以"看到"HPC节点的信息
- 即使本地机器关机，HPC Agent仍能独立工作

### 步骤1：在HPC上安装OpenClaw

```bash
# SSH登录HPC
ssh user@hpc.example.ac.jp

# 安装Node.js（如果没有的话，用nvm）
...
> ⚠️ **HPC注意事项**
> - 有些HPC的登录节点是x86_64，计算节点是ARM（如aarch64）
> - OpenClaw Gateway跑在**登录节点**上（不消耗计算资源）
> - 登录节点通常有网络访问权限（可连接Telegram API）
> - 如果登录节点没有网络，需要配置代理
...
```json
{
  "agents": {
    "main": {
      ...
      "workspace": "/home/user/.openclaw/agents/main/agent"
    }
  },
  "channels": {
    "telegram": {
      "botToken": "YOUR_HPC_BOT_TOKEN",
...
**关键配置**：
- **端口用18790**：避免与本地Gateway的18789冲突（如果通过SSH隧道访问）
- **模型选Sonnet**：HPC任务主要是执行命令，不需要最贵的Opus
- **workspace用本地路径**：Agent的AGENTS.md写上HPC特有的信息（项目路径、SLURM模板等）

### 步骤3：用tmux保持Gateway运行

HPC的SSH连接会超时断开，必须用tmux保持Gateway在后台运行：
...
# 在tmux中启动Gateway
tmux send-keys -t openclaw-gw 'openclaw gateway start' Enter

# 确认运行状态
tmux send-keys -t openclaw-gw 'openclaw gateway status' Enter

# 断开SSH后，Gateway继续运行
# 下次SSH登录时重新attach
tmux attach -t openclaw-gw
```

### 步骤4：配置HPC Agent的"人格"

在HPC workspace的`AGENTS.md`中写入HPC特有知识：

```markdown
# HPC Agent - Wisteria

你是一个运行在Wisteria超算上的Agent。

## 环境信息
- 系统：RHEL 8.x (aarch64计算 / x86_64登录)
- Job调度：SLURM (pjsub)
- GPU：NVIDIA A100 80GB × 4 per node
- 项目目录：/work/02/gs58/d58004/project/
...
source /work/project/venv/bin/activate
python train.py
\```

## 常用命令
- 提交job: `pjsub script.sh`
- 查看队列: `pjstat`
- 取消job: `pjdel JOB_ID`
- 查看GPU: `nvidia-smi`（需在计算节点上）
```

### 步骤5：建立Node连接

Node连接让本地Gateway"看到"远程HPC节点。在本地Gateway配置中添加：

```bash
# 在HPC上获取node信息
openclaw node status
# → Node ID: dd4f50bed01917a891e26fb435f093af...
...
```
你 → @HPCBot：
  "帮我在MNIST数据集上跑一下fastUMAP，用默认参数"

HPC Agent：
  📋 准备提交实验
  - 数据集：MNIST (70000 × 784)
  - 算法：fastUMAP (ARPACK + GPU)
  - 预计时间：~15分钟
...
HPC Agent：
  ...
  - Job 8400682：成功
  - 运行时间：12分34秒
  - 输出文件：/work/project/results/mnist_fastumap.npz
  - 残差范数：1.2e-7（收敛良好）
...
1. **模型选择**：不要在HPC上用需要大量API调用的模型。HPC网络可能不稳定，选择能容忍高延迟的provider
2. **tmux必须**：没有tmux就没有持久Gateway。SSH断开 = Gateway停止
3. **停机维护**：HPC经常有定期维护（如每月一次），Gateway会被中断。维护后需要手动重启
4. **存储路径**：HPC的存储通常是`/work/`或`/scratch/`，不是`/home/`。确保workspace在正确的路径下
5. **防火墙**：某些HPC限制出站网络。确认登录节点可以访问Telegram API（443端口）

> 💡 **AI辅助提示**
> 不熟悉SLURM或HPC？问AI：
> "什么是SLURM job调度系统？如何写一个基本的SLURM提交脚本？"
> 或者："HPC超算的登录节点和计算节点有什么区别？"
